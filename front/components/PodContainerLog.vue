...
export default {
  ...
  async fetch() {
    ...
    const params = new URLSearchParams({ pod: this.pod, container: this.container, timestamps: this.timestamps })
    const result = await fetch(`/api/${this.$route.params.environmentID}/pod-container-logs?${params}`);
    if (result.ok) {
